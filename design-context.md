...
- **Ops user research team cards (2026-09-14):** the Pre-Auth and Claim Team cards put the title on its own line with the tags on a row beneath it (inline, Claim Team's tags needed 506px of a 504px header at 1440 and squeezed the title onto two lines). Each tag is `white-space:nowrap` so it wraps whole. The two-card `.team-row` stacks to one column at ≤1100px. Both cards are flex columns with the bullet panel `flex:1`, so the tint fills to the bottom. Supervisors keeps title and tags on one row, wrapping when narrow. The emphasis in each card's bottom note is bold in the sentence's own colour, never the accent blue. The journey map below them is `images/ops-claim-lifecycle-journey-map-v2.webp` (2400×1494, light artwork: ten stages, mood line, pain points, the handoff at discharge approval; raw in `_originals/Claim Lifecycle Journey Map-Ops.png`); the old dark 2000×725 map was deleted. Known issue: Ops sections carry `padding:80px 48px` inline with no mobile override, so at 390px the content column is ~280px and the journey map renders ~174px wide.
