...
- IHX family, so indigo/teal. Built on User Management's head, chrome, footer and scripts, with the §11c hero block in order: tags `IHX Case Study · B2B · Mobile App · Health InsurTech`, title "Turning the TPA Desk Back Into a TPA Desk", lede, `.hero-image` placeholder (`images/nucleus-mobile-hero.webp`), project info `Role · Team · Platform · Timeframe` = `Product Design · UX · UI · Prototyping`, `1 PM · Engineering leads · 1 PD (me)`, `Android · MVP`, `3 weeks · design in 2 days`; metrics `9,500+` daily active users (non-TPA hospital staff), `↑33%` more client acquisition, `7 roles` brought into the claim (the PRD's seven personas), then the five-card TL;DR.
...
- **Second pass (2026-09-14), from Shabnam's feedback and the real UI:** 01 is "Patient Journey" ("The patient journey inside a hospital for a cashless claim"). 02 opens with `.q-grid`: the families' three questions (approved? how much? out of pocket?) and what the insurer asks for, set large in Staatliches, before the problem cards. 03 is "Designed For": the four moments plus three constraints in Shabnam's words: (1) no training needed, tiles that answer at a glance, for staff of varying tech literacy and language fluency; (2) low-network zones inside hospitals, tier 2 and 3 connectivity and low-end phones, so a UI light enough to load fast; (3) always current, so answers about the claim and bill are precise and empathetic. 04 Scope is in/out only, with Global search added to "left out" (planned next version). 05 Design decisions now describe the real UI: dashboard tiles (Active Claims, Approvals Pending, Open Queries, Discharges Due initiated/awaiting, Claims Settled settled/pending, hospital switcher, bottom bar Dashboard/Search/Claims List), claim cards (approved amount large, colour-coded status), claim details in three grouped cards plus Latest Update with insurer remarks, Open Queries with Discharge Due Today / Approvals Pending filters and a respond screen (document pdf/jpg/png within payer size limit, plus remarks), and structured search (patient name, mobile, member ID, registration no., IP number, doctor name). Global search was the goal but restrictions at the time meant the MVP shipped structured search. There is no Drafts tile in the real UI.
- **Real mockups in `images/` (2x WebP renders of Shabnam's 360-wide exports, originals in `_originals/nucleus-mobile/`):** `nucleus-mobile-dashboard`, `-claims-list`, `-claim-details`, `-open-queries` (new slot), `-query-respond`, `-search`. Added the same day: `-login`, `-forgot-password` (the reset-link confirmation shows inline on the login screen, with Resend), `-documents` (file list with upload time, view and download), `-timeline` (status history with remarks and times). Still placeholders: `hero`, `journey-map`, `user-flow`. The read-only query slot was dropped in favour of the open queries list, and the profile slot was removed because no profile screen was supplied; Supporting screens is now login and password reset only.
...
- **Linked (2026-09-14).** Home page order is now AI at the Point of Human Failure, IHX Nucleus Mobile, IHX Nucleus Ops Dashboard, WiseWoman, then behind "View more": User Management and IHX OneHealth (still "in progress"). The Nucleus Mobile index card is a real link with the default "View case study" cursor. Every case study's Other Case Studies grid lists the other four in that same order (2 × 2), never itself.
