...
- **AI at the Point of Human Failure** (`ihx-ai-features-case-study.html`). Structure since 2026-09-13: §01 is IHX MIRA, opening on the real `images/ihx-mira-claim-conversation.mp4` (1280×722, 57s), then a linked `a.node` flow breaking the conversation into the four modules, the capability callout and a plain status callout (MIRA is a prototype, modules are live). §02–05 are BenefSnap, CodeRight, DocuSense, QueryLens, each with its "Module walkthrough" video slot directly under the section intro (files still pending: `images/benefsnap-upload-autofill.mp4`, `images/coderight-diagnosis-search.mp4`, `images/docusense-checklist.mp4`, `images/querylens-categorization-chat.mp4`). The old bottom MIRA section is gone. Earlier note: the page had no real screenshots yet — every screen reference uses the existing `.video-slot`/`.img-slot` fallback pattern (dashed placeholder + expected filename), consistent with its "designed / in development" status rather than pretending finished visuals exist. Its hero also has no single screenshot to frame (four features at four different stages), so the hero's right column is a `.status-card`/`.status-row`/`.status-pill` component (new, not shared with other case studies) listing each feature's ship status instead of a framed screen.
...
- Videos are H.264 MP4 and were left as exported: macOS `avconvert` could not shrink them further. Re-encode with ffmpeg if it is ever installed; the WiseWoman scheduling clip (3.5 Mbps) is the first candidate.
- TL;DR: all four case studies now open with the five-card TL;DR right after the hero metrics. Ops and WiseWoman got theirs on 2026-09-13, styled with each page's own section header and accent (indigo, violet).
