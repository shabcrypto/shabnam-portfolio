...
- Habit Reflection card image: `images/wisewoman-habit-reflection.png`, the Your Activities phone cropped out of its 4500×3000 transparent canvas (`activity dashboard.png`) and scaled to 1200px tall, transparency kept, shown with `contain` so the phone sits centred on the dark card.
- Block (7), UI showcase: one full-width 3:2 image closing section 06, directly above section 07. `images/wisewoman-ui-showcase.jpg`, 2400×1600 web export of the 4500×3000, 11 MB `UIshowcase.png`, `object-fit: cover`, radius 20px, card shadow once loaded. Uses the shared `.exp-asset` slot.
- Task Creation card: assets supplied are a 3:2 still of three Add Task phones (`images/wisewoman-scheduling.jpg`, from `schdeuling.png`) and an 8.2s 720×1074 phone recording of Today's Schedule (`images/wisewoman-scheduling.mp4`, from `scheduling.mp4`). Decided by Shabnam: the still goes in the card as a 3:2 slot (`.exp-feature-shot.is-wide`, `cover`, radius 14px), and the recording sits in the text column under the two paragraphs as a phone-sized loop (`.exp-feature-video`: max 260px wide, 720:1074, radius 24px, black, card shadow, autoplay muted loop playsinline). The page's reduced-motion script pauses it along with the other looping videos. The earlier `images/wisewoman-task-capture.png` slot name is retired.
